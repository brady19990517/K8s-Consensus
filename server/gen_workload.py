...
def gen_workload(min_w:int, max_w:int, length:int, job_scheduling:bool, real_workload:bool):
    ...
    if real_workload:
        ...
        if job_scheduling:
            # 1 task need 100 
            workload = np.random.randint(1,10, size=[length,1])*100
        else:
            workload = np.random.randint(min_w,max_w+1, size=[length,1])
        total_task = 0
